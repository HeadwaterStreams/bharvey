...
def td_cmd_00(tool_cmd):

    import subprocess

    cmd = "mpiexec -n 8 " + tool_cmd
    subprocess.Popen(cmd, shell=False, stdout=subprocess.PIPE)


def td_cmd_01(tool_args):
...
def area_d8_00(p_path):
    ...
    #td_cmd_00("AreaD8 -p {p} -ad8 {a}".format(p=p_path, a=d8))
...
def grid_net_00(p_path):
    ...
    #td_cmd_00("Gridnet -p {p} -plen {l} -tlen {t} -gord {g} -thresh {
    # t}".format(p=p_path, l=plen_path, t=tlen_path, g=gord_path))
...
#####################################
# Experimental, don't use these yet.
...
def threshold_00(ssa_path, threshold):
    ...
    # td_cmd_00("Threshold -ssa {i} -src {o} -thresh {t}".format(i=ssa,
    # o=src_path, t=threshold))
...
def stream_net_00(fel_path, p_path, ad8_path, src_path):
    ...
    # td_cmd_00("StreamNet -fel {f) -p {p} -ad8 {a} -src {s} -ord {o} -tree {t} "
    #           "-coord {c} -net {n}{x}".format(f=fel_path, p=p_path,
    #                                           a=ad8_path, s=src_path,
    #                                           o=ord_path, t=tree_path,
    #                                           c=coord_path, n=snet_grp,
    #                                           w=bsn_path, x=sw))
...
#######################################################################
# === Run these after using other tools to produce the stream net === #

def inverse_plan_to_snet_00(invplan_path, threshold, fel, p, d8):  # FIXME
    """Create flow weighted, stream src, & stream net groups from planform

    """

    src = threshold_00(invplan_path, threshold)
    rch = stream_net_00(fel, p, d8, src)
